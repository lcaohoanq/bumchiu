...
EntityCollision.arcToWall = function (
  arcX,
  arcY,
  arcRadius,
  wallX,
...
var Player = function (x, y) {
  ...
  this.update = function () {
    ...
    if (
      (Math.abs(this.damageVelocity.x) !=
        0 < Math.abs(this.damageVelocity.y)) !=
      0
    ) {
      ...
      if (
        Math.abs(this.damageVelocity.x) < 0.5 &&
        Math.abs(this.damageVelocity.y) < 0.5
      ) {
        this.damageVelocity = { x: 0, y: 0 };
        this.canTakeDamage = true;
      }
...
var Enemy = function (x, y) {
  ...
  this.update = function () {
    ...
    var bounds = {
      x: this.x - arcSizeRadius,
      y: this.y - arcSizeRadius,
      width: arcSizeRadius * 2,
      height: arcSizeRadius * 2,
    };
    ...
    if (
      Math.abs(this.pushBulletVelocity.x) < 0.5 &&
      Math.abs(this.pushBulletVelocity.y) < 0.5
    ) {
      this.canBePushedByBullet = true;
...
var Bullet = function (i) {
  this.vectorX = Math.cos(
    player.angle + (90 * Math.PI) / 180 + (i * 5 * Math.PI) / 180
  );
  this.vectorY = Math.sin(
    player.angle + (90 * Math.PI) / 180 + (i * 5 * Math.PI) / 180
  );
